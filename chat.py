...
def mark_online(user_id):
	...
	all_users_key = 'urdu-online-users/%d' % (now // 60)
	user_key = 'urdu-user-activity/%s' % user_id
	p = red.pipeline()
...
def get_user_last_activity(user_id):
	last_active = red.get('urdu-user-activity/%s' % user_id)
...
def get_online_users():
	...
	return red.sunion(['urdu-online-users/%d' % (current - x) for x in minutes])
